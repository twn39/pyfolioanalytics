    ...
    for obj in objectives:
        ...
        if obj_name in ["VaR", "mVaR"]:
            ...
            method = (
                "modified"
                if obj_name == "mVaR"
                or obj.get("arguments", {}).get("method") == "modified"
                else "gaussian"
            )
...
def equal_weight(R: pd.DataFrame, portfolio: Portfolio, **kwargs) -> Dict[str, Any]:
    ...
    measures = calculate_objective_measures(
        weights, moments, portfolio.objectives, R=R.values
    )
    return {
        "weights": pd.Series(weights, index=portfolio.assets.keys()),
...
def inverse_volatility_weight(
    ...
    moments = set_portfolio_moments(R, portfolio)
    measures = calculate_objective_measures(
        weights.values, moments, portfolio.objectives, R=R.values
    )
    return {
        "weights": pd.Series(weights, index=portfolio.assets.keys()),
        "objective_measures": measures,
    ...
    **kwargs,
    ...
    if isinstance(portfolio, MultLayerPortfolio):
        ...
        for meta_asset, sub_port in portfolio.sub_portfolios.items():
            res_sub = optimize_portfolio(
                R, sub_port, optimize_method=optimize_method, **kwargs
            )
            if res_sub["weights"] is not None:
                sub_weights[meta_asset] = res_sub["weights"]
                sub_returns[meta_asset] = (
                    R[list(sub_port.assets.keys())] @ res_sub["weights"].values
                )
            else:
        ...
        res_root = optimize_portfolio(
            R_meta, portfolio.root, optimize_method=optimize_method, **kwargs
        )
        if res_root["weights"] is not None:
            ...
            moments = set_portfolio_moments(
                R, Portfolio(assets=list(final_weights.keys()))
            )
            measures = calculate_objective_measures(
                w_series.values, moments, portfolio.root.objectives, R=R.values
            )
            return {
                "weights": w_series,
                "objective_measures": measures,
    ...
    if optimize_method == "random":
        ...
        if best_w is not None:
            ...
            measures = calculate_objective_measures(
                best_w, moments, objectives, R=R.values
            )
            return {
                "weights": weights,
                "objective_measures": measures,
    ...
    if optimize_method == "HRP":
        ...
        measures = calculate_objective_measures(
            w_hrp.values, moments, objectives, R=R.values
        )
        return {
            "weights": w_hrp,
            "objective_measures": measures,
    ...
    if optimize_method == "HERC":
        ...
        measures = calculate_objective_measures(
            w_herc.values, moments, objectives, R=R.values
        )
        return {
            "weights": w_herc,
            "objective_measures": measures,
    ...
    if optimize_method == "NCO":
        ...
        measures = calculate_objective_measures(
            w_nco.values, moments, objectives, R=R.values
        )
        return {
            "weights": w_nco,
            "objective_measures": measures,
    ...
    if any(obj["name"] == "EVaR" for obj in objectives if obj.get("enabled", True)):
        ...
        result = solve_cla(moments, constraints, objectives, **kwargs)
    else:
        has_risk_budget = any(
            obj["type"] == "risk_budget"
            for obj in objectives
            if obj.get("enabled", True)
        )
        if has_risk_budget or optimize_method == "DEoptim":
            if optimize_method == "DEoptim":
                result = solve_deoptim(
                    moments, constraints, objectives, R=R.values, **kwargs
                )
            else:
    ...
    if result["weights"] is not None:
        ...
        measures = calculate_objective_measures(
            weights.values, moments, objectives, R=R.values
        )
        return {
            "weights": weights,
            "objective_measures": measures,
...
def create_efficient_frontier(
    ...
    **kwargs,
    ...
    if res_min["weights"] is None or res_max["weights"] is None:
        raise ValueError("Could not bound frontier.")
    ...
    for target in target_returns:
        ...
        if res["weights"] is not None:
            row = {
                "mean": res["objective_measures"]["mean"],
                "StdDev": res["objective_measures"]["StdDev"],
            }
