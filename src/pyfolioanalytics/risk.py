...
def VaR(
    ...
    if method == "gaussian":
        ...
        p_kurt = np.dot(
            weights, np.dot(m4, np.kron(weights, np.kron(weights, weights)))
        ) / (p_sd**4)
        h_p = (
            z_p
            + (1 / 6) * (z_p**2 - 1) * p_skew
            + (1 / 24) * (z_p**3 - 3 * z_p) * (p_kurt - 3)
            - (1 / 36) * (2 * z_p**3 - 5 * z_p) * (p_skew**2)
        )
...
def risk_contribution(
    weights: np.ndarray, sigma: np.ndarray, name: str = "StdDev"
) -> np.ndarray:
    if name in ["StdDev", "var"]:
        ...
        raise NotImplementedError(
            f"Risk measure '{name}' is not supported for contribution."
        )
