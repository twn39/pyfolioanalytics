...
class CLA:
    ...
    def _get_matrices(self, f: List[int], w: np.ndarray) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    ...
    def _compute_lambda(self, covarF_inv: np.ndarray, covarFB: np.ndarray, 
                        meanF: np.ndarray, wB: np.ndarray, i: int, bi: Any) -> Tuple[float, float]:
    ...
    def _compute_w(self, covarF_inv: np.ndarray, covarFB: np.ndarray, 
                   meanF: np.ndarray, wB: np.ndarray, lam: float) -> Tuple[np.ndarray, float]:
        onesF = np.ones((len(meanF), 1))
        ...
        while True:
            ...
            if len(f) > 1:
                ...
                for j, idx in enumerate(f):
                    lam, bi = self._compute_lambda(covarF_inv, covarFB, meanF, wB, j, [self.lb[idx].item(), self.ub[idx].item()])
            ...
            if len(b) > 0:
                for idx in b:
                    ...
                    covarF_inv = np.linalg.inv(covarF)
                    lam, bi = self._compute_lambda(covarF_inv, covarFB, meanF, wB, len(f_temp)-1, w[idx].item())
                    
                    if (self.ls[-1] is None or lam < self.ls[-1]) and lam > self._infnone(l_out):
                        l_out, i_out = lam, idx
            ...
            if self._infnone(l_in) < 0 and self._infnone(l_out) < 0:
                ...
                covarF, covarFB, meanF, wB = self._get_matrices(f, w)
                covarF_inv = np.linalg.inv(covarF)
                wF, g = self._compute_w(covarF_inv, covarFB, np.zeros(meanF.shape), wB, 0.0)
            else:
    ...
    def _purge_num_err(self, tol: float):
        ...
        while i < len(self.w):
            w = self.w[i]
            if abs(np.sum(w) - 1.0) > tol or np.any(w < self.lb - tol) or np.any(w > self.ub + tol):
    ...
    def max_sharpe(self, risk_free_rate: float = 0.0) -> np.ndarray:
        ...
        for i in range(len(self.w) - 1):
            res = minimize_scalar(sr_func, bounds=(0, 1), args=(self.w[i], self.w[i+1]), method='bounded')
    ...
    def efficient_frontier(self, points: int = 100) -> Tuple[np.ndarray, np.ndarray, List[np.ndarray]]:
        if not self.w: self.solve()
        ...
        if n_segments <= 0:
            w = self.w[0]
            return np.array([(w.T @ self.mu).item()]), np.array([np.sqrt((w.T @ self.sigma @ w).item())]), [w.flatten()]
            
        points_per_segment = max(2, points // n_segments)
        
        for i in range(n_segments):
            alphas = np.linspace(0, 1, points_per_segment)
            if i < n_segments - 1: alphas = alphas[:-1] # avoid duplicate points
