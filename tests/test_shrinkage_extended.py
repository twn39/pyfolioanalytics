...
def test_shrinkage_identity(stocks_data):
    ...
    moments = set_portfolio_moments(R, port, method="shrinkage", shrinkage_target="identity")
...
def test_shrinkage_constant_correlation(stocks_data):
    ...
    moments = set_portfolio_moments(R, port, method="shrinkage", shrinkage_target="constant_correlation")
    assert "sigma" in moments
    assert moments["sigma"].shape == (5, 5)
