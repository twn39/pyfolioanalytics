...
def test_frontier_on_real_data(stock_returns):
    ...
    assert np.all(np.diff(returns) > 1e-7), (
        "Frontier returns must be strictly increasing"
    )
    assert np.all(np.diff(risks) > 1e-7), (
        "Frontier risks (StdDev) must be strictly increasing"
    )

    # 2. Feasibility check: The sum of weights must equal 1 for every portfolio on the frontier
    weight_cols = [
        col for col in frontier.columns if col not in ["mean", "sd", "StdDev"]
    ]
